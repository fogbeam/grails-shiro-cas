...
"""
    ...
    def doWithSpring = {
        ...
        if (!securityConfig.filter.config) {
            ...
            if (ConfigUtils.getCasEnable()) {
                casFilter(org.apache.shiro.cas.MyCasFilter) {bean->
                    if (securityConfig.cas.failureUrl) {
                        failureUrl = securityConfig.cas.failureUrl
                    }
                    if (securityConfig.cas.logoffUrl) {
                        logoffUrl  = securityConfig.cas.logoffUrl
                    }
                }
            }
    ...
    boolean accessControlMethod(filter, boolean authcRequired, Map args = [:], Closure c = null) {
        ...
        if (subject.principal == null || (authenticatedUserRequired && !subject.authenticated) || request.getSession(false)==null) {
            ...
            if (doDefault) {
                ...
                if(!targetUri.equals("/")){
                    ...
                    filter.session['targetUri']= targetUri
...
//                    targetUri << "shiroPostRedirect=1"
                ...
                if (redirectUri) {
                    ...
                    if (redirectUri) {
                        ...
                        def sess = request.getSession(false)
                        if(sess==null ||!sess.isNew()) {
                            filter.redirect(uri: ConfigUtils.getLogoffUrl())
                            print "Will redirect to "+ ConfigUtils.getLogoffUrl()
                        }else{
                            filter.redirect(uri: ConfigUtils.getLoginUrl())
                            print "Will redirect to "+ ConfigUtils.getLoginUrl()
                        }
                    }
                }
                else {
    ...
    def processAnnotations(controllerClass, roleMap, permissionMap, log) {
        ...
        clazz.declaredFields.each { field ->
            ...
            if (ann != null) {
                ...
                def constructor = ann.type().getConstructor([ String ] as Class[])
                permissions << constructor.newInstance([ann.actions() ] as Object[])
            }
        }
    }
}
