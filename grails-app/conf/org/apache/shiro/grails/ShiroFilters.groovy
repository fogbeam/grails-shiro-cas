...
class ShiroFilters {
    def shiroAnnotationHandlerService
                    static boolean authFirst = false;
    def filters = {
        ...
        accessControlCheck(controller : '*', action : '*') {
            before = {
                ...
                if (grailsApplication.config.security.shiro.annotationdriven.enabled) {
                    ...
                    def noNeedAuthorizeControllers = ["auth"]
                    def applicationNoNeedControllers = grailsApplication.config.security.shiro.noneedauthorize.controllers
                    if(applicationNoNeedControllers){
                        noNeedAuthorizeControllers = noNeedAuthorizeControllers = applicationNoNeedControllers
                    }
                    if(controllerName in noNeedAuthorizeControllers){
                        return true
                    }
